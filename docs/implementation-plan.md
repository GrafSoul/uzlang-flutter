...
- [x] Настройки урока (шит, шестерёнка): письменность/время на лету ✅ (реализован в Ф11, 1:1 Figma)
...
- [x] Жизни: дневной пул 3 шт. (LivesService), восстановление В ПОЛНОЧЬ (не через
      24ч), «Нет жизней» показывает честный отсчёт ЧЧ:ММ до нового дня
- [x] Светлая тема (по решению владельца, палитра производная от тёмной):
      AppColors → переключаемые палитры, стили/тема/системные бары динамические,
      смена в профиле применяется мгновенно (`Get.forceAppUpdate`)
- [x] Настройки урока (шит с шестерёнки в уроке, 1:1 Figma `42:2`): письменность
      на лету (ScriptDisplay на карточках Учить/Тест/Повтор/Фразы) + время на урок
- [x] Недельный график: история XP по дням (таблица `daily_activity`, схема v3,
      GamificationService пишет при начислении)
- [x] Светлая тема (реальные светлые токены) ✅ — см. выше
